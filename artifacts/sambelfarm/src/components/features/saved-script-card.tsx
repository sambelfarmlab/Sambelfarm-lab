...
import { Edit3, RefreshCw, Sparkles, Trash2, Palette, FileDown, Tag } from "lucide-react";
import {
  type NotionPage,
  getTitle,
  getRichText,
...
function statusBadgeClass(status: string) {
  if (status === "Dipublikasi") return "bg-emerald-100 text-emerald-700 border-emerald-200";
  if (status === "Final") return "bg-primary/10 text-primary border-primary/20";
  if (status === "Terjadwal") return "bg-sky-100 text-sky-700 border-sky-200";
  if (status === "Revisi") return "bg-amber-100 text-amber-700 border-amber-200";
...
interface Props {
  ...
  onChangeStatus: (page: NotionPage) => void;
